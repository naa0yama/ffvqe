...
| ------: | ---------: | ------------: | ------------: |
|    Size |            |               |               |
|    720p |     395 MB | 280 MB ( 70%) | 274 MB ( 69%) |
|   1080p |     762 MB | 467 MB ( 61%) | 439 MB ( 57%) |
|         |            |               |               |
| bitrate |            |               |               |
...
> [!NOTE]
> MPEG-2 TS で 10/15Mbps の場合、 1920x1080 にリサイズすると Bitrate が不足しリサイズでブロックノイズやモスキートノイズも引き伸ばされることになりメリットがない。 1280x720 にダウンスケールすると Bitrate は十分に確保でき画素密度が 69% 向上する。 タブレットでの視聴を想定する場合でも 1280x720 の解像度で十分な画素間を維持することができる。 1440x1080 の解像度で 横をスケーリングすると 960x720 という解像度もあるが、再生時にソフトウェア的に 16:9 にするため 25% の画素を補完することで表示する。この処理で劣化を目立って確認することになるので素直に 1280x720 が最適解であった。

## VMAF

VMAF の高速化のために確認した\
VMAF 3.0 から `n_threads` を設定しない場合は `n_threads=1` となる。これは ffvqe でコア数を自動確認し適用している。 `n_subsample=1` を `n_subsample=3` などに変更すると 3フレーム毎に VMAF を計測するようにでき、フレームをサンプルして計測するため高速化できる。\
libsvtav1 でテストした限り、 VMAF の振れ幅(n_subsample=X)を利用することでの測定誤差は下記

### n_subsample の統計確認

合計144データポイント

| n_subsample | 平均実行時間 | 削減率 | 最大絶対誤差 | 平均絶対誤差 |   誤差率 |
| ----------: | -----------: | -----: | -----------: | -----------: | -------: |
|           1 |      89.52秒 |   0.0% |          `-` |          `-` | (基準値) |
|           2 |      52.69秒 |  41.1% |     0.201204 |     0.048338 | ± 0.212% |
|           3 |      40.50秒 |  54.8% |     0.153355 |     0.053164 | ± 0.161% |
|           4 |      35.00秒 |  60.9% |     0.206015 |     0.049843 | ± 0.217% |
|           5 |      32.21秒 |  64.0% |     0.284158 |     0.071986 | ± 0.299% |
|           6 |      30.04秒 |  66.4% |     0.235370 |     0.064093 | ± 0.248% |

- `n_subsample=5`, `6` を採用しない理由
  - `n=5` の問題
    - 最大誤差: ±0.299%(n=3の1.9倍)
    - 時間削減: n=4 から 3.2% のみ(効果飽和)
    - 平均誤差も1.4倍に増大
    - アニメなどで 24fps を 29.97fps にテレシネする場合は 5fps 間隔で上下フィード違うフレームの可能性がある
  - n=6の問題
    - 最大誤差: ±0.248%(n=3の1.5倍)
    - 時間削減: n=5 から 2.2% のみ(限界的)
    - 83%フレームスキップ(統計的信頼性低)
- 推奨: `n_subsample=3`
  - 誤差: ±0.161%(許容範囲)
  - 高速化: 54.8%(十分)
